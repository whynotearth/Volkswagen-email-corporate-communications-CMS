        ...
        <div class="px-4 pt-4">
          <div class="mb-8" v-for="item in list" :key="item.id">
            <router-link
              ...
              :to="{ name: pageInfo.listItemDetailPath, params: { id: item.id } }"
            >
              <ActivityListItem :model="item" />
            </router-link>
          </div>
        </div>
      </div>
...
export default {
  ...
  components: { ActivityListItem, BaseAppBarHeader, LayoutFixedScrollableWithNav, NavigationBottom }
};
</script>
